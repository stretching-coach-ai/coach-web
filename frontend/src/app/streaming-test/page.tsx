...
export default function StreamingTestPage() {
  ...
  const handleConversationSubmit = async () => {
    ...
    if (!regularResponse && !streamingResponse) {
      alert('먼저 초기 스트레칭 정보를 요청해주세요.');
      return;
    }
    
    // 후속 질문 길이 검증
    if (followUpQuestion.trim().length < 3) {
      alert('질문은 3자 이상 입력해주세요.');
      return;
    }
    ...
    try {
      console.log('대화 컨텍스트 요청 데이터:', {
        question: followUpQuestion
      });
      
      // EventSource를 사용하여 SSE 연결
      const url = `/api/v1/sessions/${sessionId}/conversation/stream`;
      console.log('대화 컨텍스트 SSE 연결 URL:', url);
      ...
      const updateDisplay = () => {
        if (displayedLength < fullResponse.length) {
          ...
          // 현재까지 받은 응답 중 displayedLength만큼만 화면에 표시
          setConversationResponse(fullResponse.substring(0, displayedLength));
      ...
      while (true) {
        ...
        if (done) {
          console.log(`대화 스트리밍 완료: 총 ${chunkCount}개 청크 수신`);
          
          // 남은 텍스트 모두 표시
          if (displayedLength < fullResponse.length) {
            setConversationResponse(fullResponse);
          }
      ...
      setConversationLoading(false);
      setConversationResponse(`오류가 발생했습니다. ${error instanceof Error ? error.message : '알 수 없는 오류'}`);
    }
  };
  ...
  return (
    <div className="p-4">
      ...
      <div className="mt-8 border-t pt-4">
        <h2 className="text-xl font-bold mb-4">대화 컨텍스트 테스트</h2>
        <p className="mb-4 text-sm text-gray-600">
          위에서 스트레칭 정보를 요청한 후, 아래에서 후속 질문을 입력하여 대화 컨텍스트 기능을 테스트할 수 있습니다.
        </p>
        
        <div className="mb-4">
          <label className="block mb-1">후속 질문:</label>
          <div className="flex gap-2">
            <textarea 
              value={followUpQuestion}
              onChange={(e) => setFollowUpQuestion(e.target.value)}
              className={`flex-1 p-2 border rounded ${
                followUpQuestion.trim().length > 0 && followUpQuestion.trim().length < 3 
                  ? 'border-red-500' 
                  : ''
              }`}
              rows={2}
              placeholder="예: 이 스트레칭은 하루에 몇 번 해야 하나요?"
              disabled={conversationLoading || !sessionId || (!regularResponse && !streamingResponse)}
            />
            <button 
              onClick={handleConversationSubmit} 
              disabled={conversationLoading || !sessionId || followUpQuestion.trim().length < 3 || (!regularResponse && !streamingResponse)}
              className="px-4 py-2 bg-indigo-500 text-white rounded disabled:bg-gray-300"
            >
              {conversationLoading ? '로딩 중...' : '질문하기'}
            </button>
          </div>
          {followUpQuestion.trim().length > 0 && followUpQuestion.trim().length < 3 && (
            <p className="text-red-500 text-sm mt-1">
              질문은 3자 이상 입력해주세요. (현재: {followUpQuestion.trim().length}자)
            </p>
          )}
        </div>
        
        <div>
          <h3 className="text-lg font-semibold mb-2">대화 응답:</h3>
          <div 
            ref={conversationRef}
            className={`border p-4 rounded ${conversationLoading ? 'bg-gray-50 border-indigo-500 border-2' : 'bg-gray-50'} h-[300px] overflow-auto whitespace-pre-wrap`}
            style={{ 
              fontFamily: 'monospace',
              lineHeight: '1.6',
              fontSize: '14px',
          ...
          >
            {conversationLoading && conversationResponse.length === 0 && (
              <div className="flex items-center text-gray-500">
            ...
            <div className="typing-container">
              {conversationResponse || (!conversationLoading && '대화 응답이 여기에 표시됩니다...')}
              {conversationLoading && conversationResponse.length > 0 && (
                <span className="inline-block animate-pulse text-indigo-500 ml-1">▌</span>
              )}
            </div>
          </div>
          <div className="mt-2 text-xs text-gray-500">
            {conversationLoading && conversationResponse.length > 0 && '실시간 스트리밍 중...'}
          </div>
        </div>
      </div>
    </div>
